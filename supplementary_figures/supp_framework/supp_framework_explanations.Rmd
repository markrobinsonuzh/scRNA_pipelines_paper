---
title: "pipeComp - Supplementary Methods"
...
On the other hand, a `r CRANpkg("drake")`-based workflow has several advantages, in particular the capacity, upon failure of a pipeline (or changes made to a step), to recover and restart from the very next upstream target. We are currently working on an `bptry`-based approach to error handling and selective recomputation, but for the moment users requiring smooth error recovery should rather rely on `r CRANpkg("drake")`.
