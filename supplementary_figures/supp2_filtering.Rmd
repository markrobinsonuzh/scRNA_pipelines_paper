...
```{r filt_more_ari, fig.width=8, fig.height=6.5}
...
pal <- colorRamp2(c(0,1,5,20,max(pc)),c("#0000FFFF", "#B38BFFFF", "#FFFFFFFF", "#FF9E81FF", "#FF0000FF"))
p2 <- grid.grabExpr(draw(Heatmap(pc, name="% out", bottom_annotation=ha, col=pal, cluster_columns=FALSE, show_column_names=FALSE)))

plot_grid(p1, p2, labels=LETTERS[1:2], nrow=2, rel_heights=c(3,2))
```
...
chm(list("standard Seurat norm"=sw[,-w], "sctransform"=sw[,w]), scale="row", scaleVar=FALSE, scaleTitle="meanSilWidth\ndifference", sameScale=TRUE, row_ann=ra, show_row_names=FALSE, cluster_rows=FALSE)
```
