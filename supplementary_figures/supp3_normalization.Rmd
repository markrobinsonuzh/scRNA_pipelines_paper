...
w <- 4*1:(ncol(sw)/4)-2
colnames(sw)[-w] <- ""
chm(list("Cluster mean silhouette width"=sw), scaleTitle="row z-score", scale="row", row_ann=ra, show_row_names=FALSE, cluster_rows=FALSE, cluster_columns=FALSE, top_annotation=HeatmapAnnotation(Filter=filter))
```
