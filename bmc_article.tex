...
\subsubsection*{Excluded more cells is not necessarily good}
...
On the basis of these observations, we defined three sets of criteria as lenient, default and stringent filtering (see methods), and investigated them further. We found that filtering had a much milder impact than doublet removal (Figure \ref{fig:figure3}), and that more stringent filtering tended to result in higher silhouette width, in a higher proportion of the variance in the first components explained by subpopulations (Supplementary Figure 10), and often in a higher adjusted Rand index (ARI) of the clustering at the right number of clusters (Figure \ref{fig:figure3}). However, part of the gain is to be ascribed to the removal of cells from subpopulations (especially in the Zheng datasets) which are difficult to distinguish, and for which more stringent clustering did not improve silhouette width (Supplementary Figure 10A). For these reasons, we therefore recommend using a more lenient filter (e.g. lenient or default filter described here) or a cluster-based filter to avoids most of the subpopulation bias while excluding strong outliers.
...
We next investigated the impact of different normalization strategies. In addition to log-normalization, the standard Seurat clustering pipeline scales features so that the PCA is not dominated by a small subset of the features, and we therefore included versions of the different normalization, with or without a subsequent scaling (except sctransform's variance-stabilizing transformation, given that, conceptually, it involves something analogous to scaling). Finally, Seurat's `ScaleData` includes the option to regress out the effect of certain covariates, and we therefore tested its use with the proportion of mitochondrial counts and the number of detected features.

An important motivation of sctransform was the observation that, even after normalization, the first principal components of various datasets tended to correlate with library size, suggesting an imperfect correlation \citep{hafemeisterSCtransform2019}. We therefore first assessed to what extent the first principal component still retained a correlation with the library size and number of detected features (Supplementary Figure 12A). In the 10X datasets, most methods successfully decreased the correlation with covariates, including the mere scaling of non-normalized data. This was not the case in the smartseq datasets, which required the additional regressing out of covariates.

(NOTE: a correlation of PC1 with library size might be caused by the fact that subpopulations have a different average library size. I'll therefore repeat this evaluation with the variance explained by libSize in a PC1\textasciitilde subpopulation+libSize model.)

We next investigated the impact of normalization on the ability to distinguish subpopulations, which was much greater than that of filtering (Supplementary Figure 13). For many clusters, scaling tended to reduce both the average silhouette width (Supplementary Figure 12B) and the proportion of variance in the top principal components explained by subpopulations (Figure \ref{fig:figure3} and Supplementary Figure 13), and was associated with a poorer adjusted Rand index (ARI) of the resulting clustering (Figure \ref{fig:figure3}). Regressing out covariates systematically gave poorer performance on all metrics. sctransform produced the highest cluster average silhouette width (Supplementary Figure 12B) as well as ARI (Figure \ref{fig:figure3}), but not always the highest mean proportion of variance in the top components explained by subpopulations (Figure \ref{fig:figure3} and Supplementary Figure 13). Interestingly, clusters with the lowest average silhouette width (and which are therefore more difficult to classify) tended to be less well distinguished with sctransform, and instead showed a good difference in non-normalized and/or non-scaled data, suggesting that some procedures emphasize differences more useful to track some subpopulations than others, and hence the possibility that further developments in methods could improve this step.
...
A standard clustering pipeline typically involves a step of selecting highly-variable genes, which is complicated by the digital nature and the mean-variance relationship of RNAseq. Seurat's earlier approaches involved the use of dispersion estimates standardized for the expression levels, while more recent versions rely of a different measure of variance, again standardized. Of note, while adjusting for the mean-variance relationship removes much of the bias towards highly-expressed genes, it is nevertheless plausible that this bias reflects biological relevance and hence is actually helpful in classifying cell types. \citep{townesGlmpca2019} instead suggested to use deviance, while sctransform provides its own ordering the genes by transformed variance. We therefore compared these methods.

Reasoning that a selection method should ideally select genes whose variability is more between subpopulations than within, we first assessed whether each selection method selected genes with a high proportion of variance or deviance explained by (real) subpopulations (the latter estimated through edgeR). Estimates of variance explained by subpopulations based on a standard Seurat normalization or on sctransform data were highly correlated (Supplementary Figure 14A), and there was also a good agreement with deviance explained, although lowly-expressed genes in particular could have a high deviance explained without having much of their variance explained by subpopulations (Supplementary Figure 14B-D). We therefore compared, using both of these estimates, the proportion of the cumulative variance/deviance explained by the top X genes that could be retrieved by each gene ranking method (Supplementary Figures 15-16). We first focused on the first 1000 genes to highlight differences between methods, although the differences tended to decrease with a higher number of genes selected (Figure \ref{fig:figure4}A). A first observation was that standardized measures of variability were systematically worse than their non-standardized  counterparts in selecting genes with a high proportion of variance explained by subpopulations, while instead proving most of the time superior in selecting genes with a high deviance explained by subpopulations (Figure \ref{fig:figure4} and Supplementary Figures 15-16). Deviance proved the method of choice to maximize the former (with mere expression level proving surprisingly good), but did not perform so well to select genes with a high deviance explained.

\begin{figure}
    \centering
    \includegraphics[scale=0.6]{{main_figures/figure4_selection_files/figure-html/figure4-1.png}}
    \caption{\textbf{Evaluation of feature selection methods. A:} Ability of different feature ranking methods to capture genes with a high proportion of variance (left) or deviance (right) explained. \texbf{B:} Accuracy of clustering (at the right number of clusters) based on the given clustering methods and number of genes selected.}
    \label{fig:figure4}
\end{figure}


We next investigated whether a selection based on these methods impacted the proportion of variance explained by subpopulations in the first principal components (Supplementary Figure 17). To validate the previous assay, included a selection based on the genes whose proportion variance or deviance explained was highest. Interestingly, while these selections were on average the top-ranking methods, they were not systematically so for all dataset.

The main observations were reflected in the ARI of the resulting clustering (Figure 4B): non-standardized measures of variability, including mere expression level, tended to outperform more complex metrics. In general, we found deviance and Seurat's unstandardized estimate of variance to provide the best results across datasets and normalization methods. Increasing the number of features selected also systematically led to an increase in the accuracy of the clustering.
...
Our new doublet detection method is available at \url{https://github.com/plger/plDoubletFinder}. We first cluster the cells using scran's \textit{quickCluster} method, favouring overclustering, and reduce the dataset to the union of 1500 most expressed genes of each cluster. We then create 75\% of the artifical doublets by sampling the two cells specifically from different clusters, and 25\% by entirely random sampling in case the clustering was erroneous, and summing their counts. We also create meta-cells from each cluster, and use them to create additional doublets and triplets. We then combine them with the real cells, convert the counts to scaled column ranks, and build a graph of the \textit{k} (default 10) nearest neighbors using scran. We then calculate, for each cell, the proportion of its neighbors that are artificial doublets. This ratio serves as a doublet score, which is then thresholded by simultaneously minimizing the error in classifying real vs artificial doublets and the deviation from the distribution of expected doublet rate (accounting for homotypic doublets as done by DoubletFinder). For the pipeline comparison, the expected doublet rate was set to 2.1\% (with a 1\% standard deviation) on the basis of the mixology datasets.

\subsection*{Filter sets}
Lenient, default, and stringent sets of filters...

\subsection*{Variance and deviance explained}

Unless specified otherwise, the variance in gene expression explained by subpopulations was calculated on the data normalized and transfored through sctransform. For each gene, we fitted a linear model using the subpopulation as only independent variable (\textasciitilde subpopulation), and used the R-squared as a measure of the variance explained. The same method was used for principal components.
...
\begin{table}[h!]
\caption{Description of the benchmark datasets (GSEids to be added)}
\label{tab:table1}
\begin{tabular}{rlrrl}
  \hline
dataset & protocol & nbCells & subpops & description \\ 
  \hline
Koh & SMARTer & 531 &   9 & FACS purified H7 hESC in different differention stages \\ 
  Kumar & SMARTer & 246 &   3 & Mouse ESC cultured in different conditions \\ 
  Zhengmix4eq & 10x & 3994 &   4 & Mixtures of FACS purified PBMCs \\ 
  Zhengmix4uneq & 10x & 6498 &   4 & Mixtures of FACS purified PBMCs \\ 
  Zhengmix8eq & 10x & 3994 &   8 & Mixtures of FACS purified PBMCs \\ 
  mixology10x3cl & 10x & 902 &   3 & Mixture of 3  cancer cell lines from CellBench \\ 
  mixology10x5cl & 10x & 3918 &   5 & Mixture of 5 cancer cell lines from CellBench \\ 
   \hline
\end{tabular}
\end{table}
