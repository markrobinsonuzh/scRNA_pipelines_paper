...
On the basis of these observations, we defined three sets of criteria as lenient, default and stringent filtering (see methods), and investigated them further. We found that filtering had a much milder impact than doublet removal (Figure 2), and that more stringent filtering tended to result in higher silhouette width, in a higher proportion of the variance in the first components explained by subpopulations (Supplementary Figure 10), and often in a higher adjusted Rand index (ARI) of the clustering at the right number of clusters (Figure 2). However, part of the gain is to be ascribed to the removal of cells from subpopulations (especially in the Zheng datasets) which are difficult to distinguish, and for which more stringent clustering did not improve silhouette width (Supplementary Figure 10A). For these reasons, we therefore recommend using a more lenient filter (e.g. lenient or default filter described here) or a cluster-based filter to avoids most of the subpopulation bias while excluding strong outliers.
