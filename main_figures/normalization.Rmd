...
p1 <- evalHeatmap(res, step="dimreduction", what=c("log10_total_counts","log10_total_features"), 
                  what2="PC1.covar.adjR2", agg.by = "norm", reorder_rows = TRUE, 
                  row_split = classifyNorms(norm), row_title="Variance in PC1 explained by covariate")
lgd <- Legend(col_fun=colorRamp2(1:100, viridisLite::inferno(100)), at=c(1,50,100), 
              title="MADs", labels=c("lowest","median","highest"))
p1 <- grid.grabExpr(draw(renameNorm(p1), merge_legend=TRUE, annotation_legend_list=lgd))
...
p3 <- grid.grabExpr(draw(
  ...
  heatmap_legend_side = "bottom", merge_legend=TRUE, annotation_legend_list=lgd))
```
