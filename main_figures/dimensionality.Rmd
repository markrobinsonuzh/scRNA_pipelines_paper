...
suppressPackageStartupMessages({
  ...
  library(matrixStats)
  library(pipeComp) # the first part requires pipeComp >0.99.26
})
...
p3 <- renameHrows(
  ...
  evalHeatmap(res, what=c("ARI"), agg.by=c("norm","dims"), row_split=gsub("norm\\.","",norm), 
              reorder_rows=TRUE, filterExpr=n_clus==true.nbClusts, 
              anno_legend=FALSE, title="ARI at\ntrue k"),
  rnfn
)
...
```{r, eval=FALSE}
# NB: the next results were generated using an older version of pipeComp (branch 
...
source("../../pipeComp_biorxiv/pipeComp/inst/extdata/scrna_alternatives.R")
```
...
plot_grid( p1, p2, p3, nrow=3, labels=c(NA,LETTERS[3:4]))
dev.off()
plot_grid( p1, p2, p3, nrow=3, labels=c(NA,LETTERS[3:4]))
```
