...
p1b <- ggplot(o2, aes(reorder(method, diff, function(x) mean(abs(x))), diff, group=method, fill=Time)) + geom_violin() + coord_flip() + geom_hline(yintercept=0, linetype="dashed") + ylab("Difference to 'real' number of dimensions") + scale_fill_manual(values=tcols) + theme(axis.title.y=element_blank(), legend.position=c(0.7, 0.3))
p1 <- plot_grid(p1a, p1b, rel_widths = c(0.42,0.58), labels=LETTERS[1:2])
```

```{r, fig.width=8, fig.height=4}
res <- readRDS("../data/results_dimensionality.rds")
sw <- res$dimensionality$clust.avg.silwidth$selected
...
row.names(sw) <- n
sw2 <- sw[order(grepl("sctransform",row.names(sw))*1000+rowMeans(sw)+apply(sw,1,probs=0.8,FUN=quantile),decreasing=TRUE),order(colMeans(sw),decreasing=TRUE)]
...
plot_grid( p1, p2, p3, nrow=3, rel_heights=c(0.39,0.3,0.31), labels=c(NA,LETTERS[3:4]))
```
